...
// Check for files that match patterns in array of items
const checkPatterns = (patterns, source) => {
  for (const pattern of patterns) {
    if (wildstring.match(pattern, source)) {
      return true
    }
  }
...
module.exports = coroutine(function*(
  ...
  for (const file of files) {
    ...
    if (checkPatterns(hiddenFiles, details.base)) {
      details.cssClass += 'hidden'
    }

    if (checkPatterns(ignoredFiles, details.base)) {
      delete files[index]
    } else {
