...
// Check for files that match patterns in ignoredFiles
const checkPatternIgnore = (ignoredFiles, fileName) => {
  for (const pattern of ignoredFiles) {
    if (wildstring.match(pattern, fileName)) {
      return true
    }
  }
...
module.exports = coroutine(function*(port, current, dir, ignoredFiles) {
  ...
  for (const file of files) {
    ...
    if (checkPatternIgnore(ignoredFiles, details.base)) {
      delete files[index]
    } else {
