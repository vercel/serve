...
module.exports = coroutine(function*(req, res, flags, current, ignoredFiles) {
  ...
  // Serve `*.md, *.markdown`, for reading docs more friendly. if:
  // - The file's extname equal 'md' OR 'markdown'
  if (
    flags.markdown &&
    ['.md', '.markdown'].indexOf(path.extname(related)) !== -1
  ) {
    // eslint-disable-next-line camelcase
    stream.mime.default_type = 'text/html'

    const markdownContent = yield renderMarkdown(pathname, related)

    // If it works, send the content listing to the user
    if (markdownContent) {
      return micro.send(res, 200, markdownContent)
    }
  }
